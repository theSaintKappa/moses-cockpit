...
export default async function NotFoundPage() {
    const image: ImageApiResponse = await fetch("https://api.saintkappa.dev/moses/pics/random", { cache: "no-cache" }).then((res) => res.json());
    ...
    return (
        <>
            ...
            <main className="flex flex-col items-center justify-center gap-8 h-supports-dvh w-screen p-4">
                <Image src={image.url} alt="404 Not Found" width={750} height={750} className="absolute size-full brightness-[0.2] pointer-events-none" />
                <div className="z-10 text-center flex flex-col gap-6 drop-shadow-[2px_2px_3px_rgb(0,0,0)]">
                    ...
                    <h1 className="text-2xl md:text-4xl font-semibold text-primary-foreground text-balance">It seems like the page you're looking for doesn't exist</h1>
                </div>
